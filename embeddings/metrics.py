...
from colpali import ColPaliEmbedder
from ranx import Run, evaluate, compare
from eval_queries import get_qrels, get_queries
import json

def run_colpali_embeddings(queries: Dict[str, str]):
    embeddings = ColPaliEmbedder()
    ...
    return Run(run, name='colpali-embeddings')
    

def run_text_embeddings(queries: Dict[str, str]):
...
def run_evaluation_example(vectordb):
    ...
    # results = evaluate(qrels, run_text, ["ndcg@5", "map@5", "mrr", "recall@10"])
    # print(results)
